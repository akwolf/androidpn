...
public class SSLConfig {
	...
	static {
		...
		keyStoreLocation = Config.getString("xmpp.ssl.keystore", "conf" + File.separator + "security" + File.separator
				+ "keystore");

		//		log.debug("xmpp.ssl.keystore is ==============>" + Config.getString("xmpp.ssl.keystore"));

		keyPass = Config.getString("xmpp.ssl.keypass", "changeit");

		trustStoreLocation = Config.getString("xmpp.ssl.truststore", "conf" + File.separator + "security"
				+ File.separator + "truststore");
		//		trustStoreLocation = Config.getString("server.home.dir") + File.separator + trustStoreLocation;
		...
		// Load keystore
		try {
			Resource keyStoreResource = new ClassPathResource(keyStoreLocation);
			keyStore = KeyStore.getInstance(storeType);
			//			keyStore.load(new FileInputStream(keyStoreLocation), keyPass.toCharArray());
			keyStore.load(keyStoreResource.getInputStream(), keyPass.toCharArray());
		} catch (Exception e) {
		...
		try {
			...
			TrustManagerFactory c2sTrustFactory = TrustManagerFactory.getInstance(TrustManagerFactory
					.getDefaultAlgorithm());
			c2sTrustFactory.init(trustStore);

			sslContext.init(keyFactory.getKeyManagers(), c2sTrustFactory.getTrustManagers(),
