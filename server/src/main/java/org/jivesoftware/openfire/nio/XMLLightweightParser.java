...
public class XMLLightweightParser {
	...
	final String[] sstatus = { "INIT", "", "HEAD", "INSIDE", "PRETAIL", "TAIL", "VERIFY", "INSIDE_PARAM",
			"INSIDE_CDATA", "OUTSIDE" };
		...
		for (int i = 0; i < readByte; i++) {
			...
			if (status == XMLLightweightParser.TAIL) {
				...
				if (ch == '"') {
					...
					status = XMLLightweightParser.OUTSIDE;
					if (insideRootTag
							&& ("stream:stream>".equals(head.toString()) || ("?xml>".equals(head.toString())) || ("flash:stream>"
